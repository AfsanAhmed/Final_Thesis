...
cv2.namedWindow("Canny_Trackbar")                         # creating trackbars for canny thresholding
...
def edgeDetector(image):                                   # Canny edge detection method
...
def laneDetector(data):                                      #ROS callback function to the /camera/image_raw topic
  ...
  rawImage = bridge.imgmsg_to_cv2(data, "bgr8")              #converting ROS image to OpenCV image
  image = np.copy(rawImage)
  gray_image = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)    # converting OpenCV image to gray scale image
  #motor_plan(gray_image[770][280], gray_image[770][520])  #Use this  for lane keeping
  

  height = image.shape[0]                                 # identify the height of the image
  canny_image = edgeDetector(gray_image)                            #calling edge detector function
  ...
  avg_lines= average_line(cropped_image,lines)          # calling average_line method
  hough_image= draw_lines(image,avg_lines)              
  newline_image =cv2.line(hough_image, (280,770),(520,770) ,255 ,5)  # just display a line in the image
  ...
  # Display images after each conversions
...
def main():                                           #Main method
  ...
  topic_name = "/camera/image_raw"                # Change according to the topic name
  img_sub = rospy.Subscriber(topic_name, Image, laneDetector) # initializing a ROS subscriber node
  rospy.spin()
